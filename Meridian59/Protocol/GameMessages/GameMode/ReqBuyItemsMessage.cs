...
{
    public class ReqBuyItemsMessage : GameModeMessage
    {
        ...
        public override int ReadFrom(byte[] Buffer, int StartIndex = 0)
        {
            ...
            Items = new ObjectID[len];
            for (int i = 0; i < len; i++)
            {
                Items[i] = new ObjectID(Buffer, cursor);
        ...
        public ReqBuyItemsMessage(uint ObjectID, ObjectID[] Items) 
            : base(MessageTypeGameMode.ReqBuyItems)
        {
            this.ObjectID = ObjectID;
            this.Items = Items;
        }

        public ReqBuyItemsMessage(byte[] Buffer, int StartIndex = 0) 
